    ...
    for(i=0; i<num_arg; i++){
        ...
        for(j = i+1; j< num_arg; j++){
            ...
            if(strcmp(cadena[i], cadena[j]) == 0){
                printf("Campos repetidos. No se pueden repetir los campos\n");
                return 1;
            }
        }
    }
...
int entrar_tabla(char *nombre_db, char *nombre_tabla){
    ...
    if(access(aux3, F_OK) == 0){
        ...
        printf(" La tabla %s no existia, creada correctamente\n", nombre_tabla);
        return 0;
    }
}
...
TABLE *seleccionarTablaOtraBD(char *nombre_tabla, char *nombre_bd_origen, char *nombre_bd_destino, TABLE *LaGranTabla, char *nombreNuevaTabla){
    ...
    tabla_aux2 = inicializar_tablas(tabla_aux->numCampos, nombreNuevaTabla, tabla_aux->campos, tabla_aux->tipos);
    ...
    crear_tabla_tab(LaGranTabla, nombre_bd_destino, nombreNuevaTabla);
    actualizar_lineas(nombre_bd_destino, LaGranLinea, tabla_aux2);
    return LaGranTabla;
